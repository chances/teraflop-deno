...
export class Signal {
   ...
   */
  static enable(force: boolean) {
    if (!force) return Signal._disableSignals > 0 ? Signal._disableSignals-- : 0;
   ...
   */
  static disconnect(object: Record<string, any>, callback: Function | Signal | {}, instance?: {}) {
   ...
   */
  static getSignals(object: Record<string, any>, out?: any[]) {
    out = out || [];
    for (const i in object) {
      const p = object[i];
  ...
  emit(...args: any[]) {
    ...
    for (const k of this.slots) {
      ...
      if (s.constructor === Signal) s.emit.apply(o, args);
      else s.apply(o, args);
    }
  }
   ...
   */
  addListener(callback: Function | Signal, object: {} | null = null) {
  ...
  /**
   * Checks if there is a bound listener that matches the criteria.
   ...
   */
  isListening(callback: Function | Signal | {}, object: {} | null = null) {
   ...
   */
  disconnect(callback: any, object?: {} | null) {
    if (
